...
export async function POST() {
  ...
  if (ownedTeams && ownedTeams.length > 0) {
    return NextResponse.json({
      ok: true,
      skipped: true,
      reason: "Owner already has a team",
    });
  }
  ...
  if (memErr) {
    return NextResponse.json(
      {
        ok: false,
        error: `Team created but owner add failed: ${memErr.message}`,
      },
      { status: 400 },
    );
  }
  ...
  const defaultContent = {
    team: {
      ...
      state: "",
      about:
        "Welcome to your new FTC team site! Edit this content in the dashboard.",
    },
    links: [
      {
        label: "What is FIRST Tech Challenge?",
  ...
  if (contentErr) {
    return NextResponse.json(
      {
        ok: false,
        error: `Site created but content failed: ${contentErr.message}`,
      },
      { status: 400 },
    );
  }
